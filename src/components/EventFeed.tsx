...
export const EventFeed: React.FC<Props> = ({ events, isLoading }) => {
  ...
  if (events.length === 0) {
    return (
      <div 
        className="text-center py-20 px-4 bg-zinc-50 rounded-3xl border-2 border-dashed border-zinc-200"
        role="region"
        aria-label="No events found"
      >
        <Sparkles size={40} className="mx-auto text-zinc-300 mb-4" aria-hidden="true" />
    ...
    >
      {events.map((event, index) => (
        ...
        >
          {event.isPriority && (
            <div 
              className="absolute top-0 right-0 bg-indigo-600 text-white text-[10px] uppercase tracking-widest px-3 py-1 rounded-bl-xl font-bold"
              aria-hidden="true"
            >
              Prioridad
            </div>
          ...
          <div className="flex justify-between items-start gap-4 text-left">
            <div className="flex-1">
              <span 
                className="text-[10px] uppercase tracking-wider font-bold text-zinc-500 mb-2 block"
              >
                {event.category}
              </span>
              <h3 
                className="text-xl font-bold text-zinc-100 mb-2 leading-tight"
              ...
              <div className="flex flex-wrap gap-4 text-xs font-medium text-zinc-500">
                ...
                <div className="flex items-center gap-1.5 bg-zinc-800 px-2 py-1 rounded-md border border-zinc-700">
                  ...
                  <span>{event.location.neighborhood} ({event.distance}km)</span>
                </div>
              </div>
            </div>
          </div>
